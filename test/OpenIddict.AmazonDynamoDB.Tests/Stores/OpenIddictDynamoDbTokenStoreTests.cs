...
public class OpenIddictDynamoDbTokenStoreTests
{
  ...
  /// <summary>
  /// Tests the new RevokeByAuthorizationIdAsync
  /// </summary>
  /// <returns></returns>

  [Fact]
  public async Task Should_RevokeTokens_Test()
  {
    // Arrange
    var context = new DynamoDBContext(_client);
    var options = TestUtils.GetOptions(new() { Database = _client });
    ...
    List<OpenIddictDynamoDbToken> OpenIddictDynamoDbTokens = new List<OpenIddictDynamoDbToken>();
    foreach (var index in Enumerable.Range(0, 10))
    {
      OpenIddictDynamoDbTokens.Add(new OpenIddictDynamoDbToken
      {
        CreationDate = DateTime.UtcNow.AddDays(-5),
        AuthorizationId = Guid.NewGuid().ToString(),
      });
    ...
    foreach (var item in OpenIddictDynamoDbTokens)
    {
    ...
    foreach (var item in OpenIddictDynamoDbTokens)
    {
      await tokenStore.RevokeByAuthorizationIdAsync(item.AuthorizationId, CancellationToken.None);
    }
    ...
    foreach (var item in OpenIddictDynamoDbTokens)
    {
      await tokenStore.DeleteAsync(item, CancellationToken.None);
    }
