...
public class OpenIddictDynamoDbApplicationStoreTests
{
  ...
  /// <summary>
  /// Tests the new members of OpenIddictDynamoDbApplicationStore 
  /// </summary>
  /// <returns></returns>
  [Fact]
  public async Task Check_ApplicationTypeSettingsAndJsonWebKeys_CreateApplication_When_ApplicationIsValid()
  {
    // Arrange
    var context = new DynamoDBContext(_client);
    var options = TestUtils.GetOptions(new() { Database = _client });
    ...
    var jwk = JsonWebKeySet.Create(str_jwk);


    await applicationStore.SetJsonWebKeySetAsync(application, jwk, default(CancellationToken));

    Assert.NotNull(application.JsonWebKeySetStr);
